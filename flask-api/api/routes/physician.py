    ...
    if session['login_type'] == 'physician':
        if request.method == 'GET':
            ...
            if physician is None:
                return WebHelpers.EasyResponse('physician with that id does not exist.', 404)
    ...
    if session['login_type'] == 'physician':
        ...
        if request.method == 'PUT':
            if physician:
                ...
                logging.warning(f'{current_user.name} updated physician with id {physician.id} name from {old_name} to {physician.name}.')
                return WebHelpers.EasyResponse(f'Name updated.', 200)

            return WebHelpers.EasyResponse(f'physician with that id does not exist.', 404)
    else:
        return WebHelpers.EasyResponse('You are not authorized to view this page.', 403)
    ...
    if session['login_type'] == 'physician':
        ...
        if request.method == 'DELETE':
            if physician:
                ...
                logging.warning(f'{current_user.name} deleted patient with id {physician_id} and name of {physician_name}.')
